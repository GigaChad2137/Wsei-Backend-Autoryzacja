...
def login(_username: str, _password: str) -> Tuple[Dict, int]:
    ...
    if auth_check != 200:
        ...
        try:
            ...
            cur = conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
            cur.execute(
                "Select id,username from users where username=%s and deleted=FALSE", (_username,))
            ...
            return {"success": True,
                    "token": token,
                    "user": {'id': user_exists[0], 'username': user_exists[1]},
...
def invalid_credentials(username: str, password: str) -> Union[int, Tuple[Dict, int]]:
    ...
    try:
        ...
        ub = cur.fetchone()
        if ub != None:
            user_object = User(ub[0], ub[1], ub[2], ub[3])
        else:
            user_object = None
